...
public class ElectronicNormalInvoiceParser {
     ...
     * @return 解析后的发票对象
     */
    public InvoiceDTO parseInvoice(File pdfFile) throws IOException {
...
//        log.debug("本地程序解析 耗时{} {} ",System.currentTimeMillis()-start,invoice);
        ...
        /*if(enableOllama) {
            // Ollama GLM-7
            start = System.currentTimeMillis();
            text = ollamaService.search(text);
        ...
        if(enableSilcom) {
            ...
            start = System.currentTimeMillis();
            text = DeepseekUtil.getInstance().processWithDeepSeek(text);
            log.debug("Silcom THUDM/GLM-4-9B-0414 解析  耗时{} {}", System.currentTimeMillis() - start, text);
        }
